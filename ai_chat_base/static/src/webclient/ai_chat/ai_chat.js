...
export class AiChat extends Component {
        ...
        if (this.props.greetUser) {
            if (this.store.length == 0){
                if (this.props.greetingMessage){
                    this.store.addMessage("assistant", this.props.greetingMessage);
                }
            }
        }
    }
    ...
    sendMessage() {
        const message = this.aiChatInputRef.el.value;
        if (message === ""){
            return;
        }
        ...
        this.store.addMessage("user", message);
        this.store.addMessage("assistant", "");

        this.props.onSendMessage(message, history);
